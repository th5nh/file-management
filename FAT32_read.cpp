...
// tim file
void findFileInRDET(LPCWCHAR drive, BYTE** entriesBuffer, DWORD entriesSize, PFAT32_BOOTSECTOR bs, BYTE* fatBuffer, DWORD fatSize, int indent, wchar_t* wPathToOpen, wchar_t* originPath, LPCWCHAR driveToOpen)
{
    ...
    for (int i = entriesSize - 1; i > 0; i--)
    {
        ...
        if (me->fileAttribute == 0x20 || (me->fileAttribute == 0x10 && entriesBuffer[i][0] != 0x2E))
        {
                    ...
                    if (me->fileAttribute != 0x10 && !wcsstr(wPathToOpenClone, L"\\"))
                    {
                        ...
                        if (fileNameWString.compare(wSplittedPathName) == 0)
                        {
                            ...
                            if (extension == L".txt") {
                                ...
                                wchar_t fullPathForFile[50] = L"\0";
                                wcscat_s(fullPathForFile, driveToOpen);
                                wcscat_s(fullPathForFile, originPath);
                                std::wifstream file(fullPathForFile);
                                std::wcout << "Full path file is: " << fullPathForFile << std::endl;
                                if (file.is_open()) {
                                    std::wstring line;
            ...
            if (me->fileAttribute == 0x10)
            {
                ...
                findFileInRDET(drive, entriesBuffer, entriesSize, bs, fatBuffer, fatSize, indent + 1, wSubPath, originPath, driveToOpen);
...
int main(int argc, char** argv)
{
    ...
    while (1)
    {
        ...
        fgetws(wPathToOpen, 102, stdin);
        wPathToOpen[wcslen(wPathToOpen) - 1] = '\0';
        wchar_t* originPathToOpen = new wchar_t[wcslen(wPathToOpen) + 1];
        wcscpy(originPathToOpen, wPathToOpen);

        findFileInRDET(wDrive, entriesBuffer, entriesSize, bs, fatBuffer, fatSize, 0, wPathToOpen, originPathToOpen, wDriveToOpen);
    }
